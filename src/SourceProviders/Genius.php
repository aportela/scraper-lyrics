...
final class Genius extends BaseProvider
{
    ...
    private function getLink(string $title, string $artist): string
    {
        ...
        $httpResponse = $this->http->GET("https://genius.com/api/search/multi", ["per_page" => 1, "q" => sprintf('"%s" "%s"', $title, $artist)]);
        if ($httpResponse->code === 200) {
            if (!in_array($httpResponse->body, [null, '', '0'], true)) {
                if ($httpResponse->is(\aportela\HTTPRequestWrapper\ContentType::JSON)) {
                    ...
                    if ($json !== null && json_last_error() === JSON_ERROR_NONE) {
                        if (is_object($json) && isset($json->meta) && is_object($json->meta) && isset($json->meta->status) && $json->meta->status == 200) {
                            if (isset($json->response) && is_object($json->response) && isset($json->response->sections) && is_array($json->response->sections) && $json->response->sections !== []) {
                                if (is_object($json->response->sections[0]) && isset($json->response->sections[0]->hits) && is_array($json->response->sections[0]->hits)) {
                                    ...
                                    $this->logger->error(\aportela\ScraperLyrics\SourceProviders\AZLyrics::class . '::getLink - Error: missing property (response->sections[]->hits[]->result->url)');
                                    ...
                                    $this->logger->error(\aportela\ScraperLyrics\SourceProviders\AZLyrics::class . '::getLink - Error: missing property (response->sections[]->hits[])');
                                ...
                                $this->logger->error(\aportela\ScraperLyrics\SourceProviders\AZLyrics::class . '::getLink - Error: missing property (response->sections[])');
                                throw new \aportela\ScraperLyrics\Exception\InvalidSourceProviderAPIResponse("Error: missing property (response->sections[])");
                            }
                        } else {
                            $this->logger->error(\aportela\ScraperLyrics\SourceProviders\AZLyrics::class . '::getLink - Error: missing property (meta->status == 200)');
                            throw new \aportela\ScraperLyrics\Exception\InvalidSourceProviderAPIResponse("Error: missing property (meta->status == 200)");
                        }
                    } else {
                        $this->logger->error(\aportela\ScraperLyrics\SourceProviders\Genius::class . '::getLink - Error: invalid JSON', [json_last_error(), json_last_error_msg()]);
                        throw new \aportela\ScraperLyrics\Exception\InvalidSourceProviderAPIResponse("Invalid JSON: " . json_last_error_msg());
                    }
                } else {
                    $this->logger->error(\aportela\ScraperLyrics\SourceProviders\Genius::class . '::getLink - Error: invalid HTTP content type', [$httpResponse->getContentType()]);
                    throw new \aportela\ScraperLyrics\Exception\InvalidSourceProviderAPIResponse("Invalid HTTP content type: " . $httpResponse->getContentType());
                }
            } else {
                $this->logger->error(\aportela\ScraperLyrics\SourceProviders\Genius::class . '::getLink - Error: empty body');
                throw new \aportela\ScraperLyrics\Exception\InvalidSourceProviderAPIResponse("Error: empty body");
            }
        } elseif ($httpResponse->code === 403) {
            // TODO: logger
            $this->logger->error(\aportela\ScraperLyrics\SourceProviders\Genius::class . '::getLink - Error: invalid HTTP response code: ' . $httpResponse->code);
                ...
                if ($domDocument->loadHTML(str_ireplace(["<br>", "<br/>", "<br />"], PHP_EOL, $response->body))) {
                    ...
                    if ($nodes !== false && $nodes->count() > 0) {
                        ...
                        if (!in_array($data, [null, '', '0'], true)) {
                            ...
                            $this->logger->error(\aportela\ScraperLyrics\SourceProviders\Genius::class . '::scrap - Error: empty lyrics');
                            throw new \aportela\ScraperLyrics\Exception\InvalidSourceProviderAPIResponse("Empty lyrics");
                        }
                    } else {
                        $this->logger->error(\aportela\ScraperLyrics\SourceProviders\Genius::class . '::scrap - Error: missing html xpath nodes', [$expression]);
                        throw new \aportela\ScraperLyrics\Exception\InvalidSourceProviderAPIResponse('Missing html xpath nodes: ' . $expression);
                    }
                } else {
                    $this->logger->error(\aportela\ScraperLyrics\SourceProviders\Genius::class . '::scrap - Error: invalid html body');
                    throw new \aportela\ScraperLyrics\Exception\InvalidSourceProviderAPIResponse("Invalid HTML body");
                }
            } else {
                $this->logger->error(\aportela\ScraperLyrics\SourceProviders\Genius::class . '::scrap - Error: empty body');
                throw new \aportela\ScraperLyrics\Exception\InvalidSourceProviderAPIResponse("Error: empty body");
            }
        } else {
            $this->logger->error(\aportela\ScraperLyrics\SourceProviders\Genius::class . '::scrap - Error: invalid HTTP response code: ' . $response->code);
